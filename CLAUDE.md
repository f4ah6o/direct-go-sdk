...
```bash
# Test every module (the repo has several; .github/ci-modules.json is the manifest)
make test
...
# Compile-check the Go snippets in README files
...
- `direct-go/COVERAGE.md` - Porting coverage status (regenerated by the coverage tool)
...
4. Run tests: `make test` (covers every module; `go test ./...` covers only the current one)
...
- Porting progress is tracked in `direct-go/COVERAGE.md`
