...
func threadKey(teamID, channelID, rootID string) string {
...
func (s *Store) GetByThread(teamID, channelID, rootID string) (ThreadMapping, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	key, ok := s.state.TeamsThreadIndex[threadKey(teamID, channelID, rootID)]
	if !ok {
...
func (s *Store) PutMapping(m ThreadMapping) error {
	...
	s.state.TeamsThreadIndex[threadKey(m.TeamID, m.ChannelID, m.RootID)] = talkKey(m.AccountID, m.TalkID)
	return s.saveLocked()
}
...
func (s *Store) Forget(accountID, talkID string) error {
	...
	if m, ok := s.state.TalkThreads[key]; ok {
		delete(s.state.TeamsThreadIndex, threadKey(m.TeamID, m.ChannelID, m.RootID))
	}
